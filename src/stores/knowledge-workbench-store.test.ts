...
const context = {
  ...
  selectedRuleIds: [],
  sourceAnchors: [],
  governanceState: 'candidate' as const,
...
const command = (overrides = {}) => ({
  schemaVersion: 'graph_interaction.v1' as const,
  commandId: 'cmd-1',
  candidateGraphId: 'candidate-1',
  acceptedReleaseId: null,
  ...
  edgeIds: ['e1'],
  dimOthers: true,
  viewport: 'fit_selection' as const,
  reason: null,
  ...overrides,
})
...
describe('knowledge workbench store', () => {
  ...
  it('atomically applies authorized commands and deduplicates command ids', () => {
    ...
    expect(
      useKnowledgeWorkbenchStore.getState().presentation.highlightedEdgeIds,
    ).toEqual(['e1'])
    expect(store.applyInteraction(command(), graph)).toBe(false)
    expect(useKnowledgeWorkbenchStore.getState().diagnostic).toBe(
  ...
  it('rejects stale and missing ids without partial mutation', () => {
    ...
    store.setContext(context)
    expect(
      store.applyInteraction(command({ candidateGraphId: 'other' }), graph),
    ).toBe(false)
    expect(
      store.applyInteraction(
        command({ commandId: 'cmd-2', edgeIds: ['missing'] }),
        graph,
      ),
    ).toBe(false)
    expect(
  ...
  it('clears focus when graph identity changes', () => {
    ...
    expect(
      useKnowledgeWorkbenchStore.getState().presentation.highlightedNodeIds,
    ).toEqual([])
  })
})
