...
describe('isCurrentRunEvent', () => {
  ...
  it('allows non-stream events through regardless of attempt_id', () => {
    expect(isCurrentRunEvent('session.info', { attempt_id: 'run-old' }, runId)).toBe(true)
    expect(isCurrentRunEvent('heartbeat', {}, runId)).toBe(true)
    expect(isCurrentRunEvent('error', { attempt_id: 'run-old' }, runId)).toBe(true)
  })
  ...
  it('allows stream events whose attempt_id matches the current runId', () => {
    expect(isCurrentRunEvent('text_delta', { content: 'hi', attempt_id: runId }, runId)).toBe(true)
    expect(isCurrentRunEvent('reasoning_delta', { content: 'thinking', attempt_id: runId }, runId)).toBe(true)
    expect(isCurrentRunEvent('attempt.completed', { summary: 'done', attempt_id: runId }, runId)).toBe(true)
    expect(isCurrentRunEvent('attempt.failed', { error: 'boom', attempt_id: runId }, runId)).toBe(true)
  })

  it('blocks stale replayed events whose attempt_id differs from the current runId', () => {
    expect(isCurrentRunEvent('text_delta', { content: 'old text', attempt_id: 'run-old' }, runId)).toBe(false)
    expect(isCurrentRunEvent('tool_result', { tool: 'x', attempt_id: 'run-old' }, runId)).toBe(false)
    expect(isCurrentRunEvent('attempt.completed', { summary: 'old done', attempt_id: 'run-old' }, runId)).toBe(false)
  })

  it('covers all gated stream event types', () => {
    const streamTypes = ['text_delta', 'reasoning_delta', 'tool_call', 'tool_progress', 'tool_result', 'attempt.completed', 'attempt.failed']
    for (const type of streamTypes) {
      expect(isCurrentRunEvent(type, { attempt_id: 'run-old' }, runId)).toBe(false)
