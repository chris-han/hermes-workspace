...
**Status:** Active UI style guideline.
**Authority:** UI style guideline only; not a runtime architecture contract.
**Scope:** Visual system, typography, color, layout, interaction style, and UI implementation guidance.
**Upstream sources:**
- [Document Authority And Versioning](../canonical/document-authority-and-versioning.md)
- [architecture.md](../canonical/architecture.md) for runtime semantics and governance boundaries

This document intentionally does **not** define semantic-governance architecture, trust-state doctrine, KGL behavior, or runtime authority models. If any UI wording or examples conflict with runtime contract semantics, preserve `architecture.md` and the relevant derived contract docs.
...
### Semantier Green Text Rule (Theme-Invariant)
**Semantier Green (`#9fe870`) always pairs with Dark Green (`#163300`) for text, regardless of theme.**

This is a **hard constraint** for brand consistency and accessibility:
- In both light and dark themes, any button, badge, or surface using Semantier Green as background **must** use Dark Green (`#163300`) for text.
- Do not use white, light gray, or theme-adaptive text colors on Semantier Green backgrounds.
- Rationale: Semantier Green has high luminance (~70% in HSL). White text on this background has insufficient contrast (WCAG AA fail). Dark Green provides consistent 4.5:1+ contrast and maintains brand recognition across theme switches.

**Implementation:**
```css
/* ✓ Correct: theme-invariant pairing */
[data-theme='semantier'] {
  --theme-accent: #9fe870;
  --theme-accent-foreground: #163300;
}
[data-theme='semantier-light'] {
  --theme-accent: #9fe870;
  --theme-accent-foreground: #163300;
}

/* ✗ Wrong: do not make accent-foreground theme-dependent for Semantier Green */
[data-theme='semantier'] {
  --theme-accent: #9fe870;
  --theme-accent-foreground: #ffffff;  /* breaks contrast and brand consistency */
}
```
...
**Location:** `hermes-workspace/public/`

| File | Format | Size | Usage | Notes |
|------|--------|------|-------|-------|
| `logo.svg` | SVG | — | Primary mark | Main brand logo asset. `rounded-button object-contain shrink-0 bg-transparent` |
| `logo.png` | PNG | — | Primary mark (raster) | Main brand logo asset (raster fallback). `rounded-button object-contain shrink-0 bg-transparent` |
| `logo-wireframe.svg` | SVG | 56×56px (`h-14 w-14`) | Welcome screen hero | Centered inside a `rounded-button bg-primary shadow-sm` container |
| `favicon.svg` | SVG | — | Browser tab icon | Standard SVG favicon |

### Additional Logo Assets (Hermes / Variant)
The following files also exist in `hermes-workspace/public/` and may be used in Hermes-specific contexts:
- `hermes-logo.png` — Hermes branding variant
- `hermes-crest.svg` — Hermes crest variant
- `hermes-avatar.png` / `hermes-avatar.webp` — Hermes avatar
...
#### No High-Luminance Text on High-Luminance Accent Backgrounds
When using accent colors (`--theme-accent`, `--semantier-green`, or any bright background) for buttons or badges, **never use white or light text** on them. White and bright accent colors often have similar luminance (明度), resulting in poor contrast especially in dark themes.

```tsx
// NG: white text on bright accent (poor contrast in dark mode)
<button style={{ background: 'var(--theme-accent)', color: '#fff' }}>

// NG: using undefined Tailwind utilities that fall back to white
<button className="bg-primary text-primary-foreground">  <!-- if --primary-foreground not defined -->

// OK: dark contrasting text explicitly defined
<button style={{ background: 'var(--theme-accent)', color: 'var(--theme-accent-foreground)' }}>
```

**Required theme tokens:** Every theme must define `--theme-accent-foreground` with sufficient contrast against `--theme-accent`. For semantier green (`#9fe870`), use dark green (`#163300`). For other accent colors, calculate a dark variant or use near-black.
...
**Tabs are not buttons.** Do not style primary view-switching tabs as filled segmented controls or CTA buttons.

**Tabs List — Line Style**
- Use the line/underline variant by default for `TabsList`
- Layout: inline `flex`; do not stretch tabs to full-width unless the product requirement is explicitly segmented
- Container: `border-b border-border`
- Background: transparent
- Radius: none on the tab rail itself
- Purpose: tabs should read as navigation within a surface, not as independent actions

**Tab Trigger — Inactive**
- Height: `h-11`
- Padding: minimal horizontal padding; prefer text-led spacing over button padding
- Radius: `rounded-none`
- Typography: `text-sm` to `text-[15px]`, `font-semibold`
...
- Background: transparent
- State signal: underline/indicator only
- Do not add filled green backgrounds for the active tab

**Tab Indicator**
**Tab Indicator**
- Placement: bottom edge of the tab rail
- Color: inherit the active tab text color by using `currentColor` (Tailwind utility: `bg-current`). This ensures the indicator matches the `data-[state=active]` text color and avoids theme tokens that may resolve transparent in certain builds.
- Thickness: 3px (use Tailwind utility `h-[3px]` for exactness)
- Motion: short translate/width transition only
- Implementation notes:
  - Ensure `overflow-visible` is set on the `Tabs` root and `TabsList` so the indicator is not clipped by parent containers.
  - Prefer animating `width` and `translate` for smooth motion; avoid animating `height` or opacity for the indicator.

**When To Use Tabs**
- Use tabs to separate alternate views or mutually exclusive methods within the same card or page section
- Login methods are a valid tab use case
- Do not use tabs when one option is the single primary action and the others are secondary buttons

**Tabs Anti-Patterns**
- Do not style tabs like `Primary Green CTA`
- Do not add `rounded-button` to tab triggers in line mode
- Do not place tabs inside a filled pill container
- Do not combine active underline and active filled background on the same control
